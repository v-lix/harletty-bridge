...
use crate::eac3_pipeline::{
    diagnose_eac3_unit, eac3_unit_can_carry_dependents, eac3_unit_carries_joc,
    eac3_unit_carries_self_contained_joc, eac3_unit_is_dependent, is_legacy_ac3_frame,
...
impl FormatBridge for AtmosBridge {
    ...
    /// dependent follows it (see `process_eac3_access_unit`), so at the end of
    /// a stream one is always held: the final 32 ms of every E-AC-3 track, and
    /// the whole of a track short enough to be one access unit. `reset` throws
    /// the same frame away, which is what a seek wants and an ending does not.
    ///
